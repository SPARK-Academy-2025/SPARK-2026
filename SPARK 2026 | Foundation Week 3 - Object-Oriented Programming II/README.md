<p align="center">
  <img src="https://img.shields.io/badge/SPARK_Academy-2026-blue?style=for-the-badge" alt="SPARK Academy 2026"/>
  <img src="https://img.shields.io/badge/Phase-Foundation-green?style=for-the-badge" alt="Foundation Phase"/>
  <img src="https://img.shields.io/badge/Week-03-orange?style=for-the-badge" alt="Week 03"/>
</p>

<h1 align="center">SPARK 2026 | Foundation Week 3</h1>
<h3 align="center">Object-Oriented Programming II</h3>

<p align="center">
  <em>Building AI capacity for medical imaging across Africa</em>
</p>

---

## Overview

Welcome to Week 3 of SPARK Academy 2026! This week opens with the Capstone Project Overview led by the Director, continues your journey into advanced medical imaging modalities, and deepens your object-oriented programming skills with OOP II — essential for building robust and scalable AI pipelines.

**This week covers three sessions:**

| # | Session | Facilitator | Format |
|---|---------|-------------|--------|
| 1 | SPARK 2026 Capstone Project Overview  | Dr. Udunna | Live |
| 2 | Introduction to Medical Imaging III | Dr. Maruf | Live |
| 3 | Introduction to Object-Oriented Programming (OOP II) |  Dr. Lukman E. Ismaila  | Live |

---

## Session 1: SPARK 2026 Capstone Project Overview

A live session led by the SPARK Academy Director, introducing the 2026 Capstone Project — outlining expectations, themes, timelines, and how participants will apply their learning to real-world medical imaging challenges.

**Click the image below to watch the recorded session:**

[![Programme Update & Director's Address](https://github.com/SPARK-Academy-2025/SPARK-2026/blob/main/SPARK%202026%20%7C%20Foundation%20Week%203%20-%20Object-Oriented%20Programming%20II/thumbnails/intro.jpg?raw=true)](https://youtu.be/JPNJrB8p5uo)

---

## Session 2: Introduction to Medical Imaging III

A continued deep dive into medical imaging modalities, building on the foundations established in previous weeks.

**Topics Covered:**
- Advanced medical imaging concepts
- Image acquisition and signal formation
- Clinical applications of imaging modalities

> 📂 **Slides:** [`SPARK2026_FDN_W03_Intro_Medical_Imaging_III.pptx`](slides/SPARK2026_FDN_W03_Intro_Medical_Imaging_III.pptx)

**Click the image below to watch the recorded session:**

[![Introduction to Medical Imaging III](https://github.com/SPARK-Academy-2025/SPARK-2026/blob/main/SPARK%202026%20%7C%20Foundation%20Week%203%20-%20Object-Oriented%20Programming%20II/thumbnails/medical.png?raw=true)](https://youtu.be/wKHQwweVKoY)

---

## Session 3: Introduction to Object-Oriented Programming (OOP II)

Building on OOP I, this session explores more advanced object-oriented concepts in Python for designing scalable AI and medical imaging pipelines.

**Topics Covered:**
- Inheritance and polymorphism
- Encapsulation and abstraction
- Practical OOP examples for medical imaging contexts

> 📂 **Slides:** [`SPARK2026_FDN_W03_OOP_II.pptx`](slides/SPARK2026_FDN_W03_OOP_II.pptx)

**Click the image below to watch the recorded session:**

[![Introduction to Object-Oriented Programming II](https://github.com/SPARK-Academy-2025/SPARK-2026/blob/main/SPARK%202026%20%7C%20Foundation%20Week%203%20-%20Object-Oriented%20Programming%20II/thumbnails/oop.png?raw=true)](https://youtu.be/Lb6zvTj9niM)

### Training Notebooks
#### OOP GitHub Repo
[![Object-Oriented Programming codes](https://img.shields.io/badge/GitHub-Object--Oriented%20Programming%20codes-black?logo=github)](https://github.com/ismailukman/FoundationalCourse_Python-OOP)


| Google Colab | Kaggle |
|:---:|:---:|
| [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1-NEpe4kCbkbAq8Ogmu8vKfVwBY8yG8YP?usp=sharing) | [![Open In Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://www.kaggle.com/code/spark2025/week-3-object-oriented-programming-oop-2) |

---

## Folder Structure

```
SPARK 2026 | Foundation Week 3 - Object-Oriented Programming II/
├── README.md
├── slides/
│   ├── SPARK2026_FDN_W03_Intro_Medical_Imaging_III.pptx
│   └── SPARK2026_FDN_W03_OOP_II.pptx
├── thumbnails/
│   ├── intro.png
│   ├── medical.png
│   └── oop.png
├── notebooks/
│   └── SPARK2026_FDN_W03_OOP_II.ipynb
```

---

## Additional Resources

**Object-Oriented Programming:**
- [Real Python - OOP in Python](https://realpython.com/python3-object-oriented-programming/)
- [Python Official OOP Tutorial](https://docs.python.org/3/tutorial/classes.html)
- [Corey Schafer - OOP Series (YouTube)](https://www.youtube.com/watch?v=ZDa-Z5JzLYM)

**Medical Imaging — MRI & PET:**
- [Radiopaedia - MRI Physics](https://radiopaedia.org/articles/mri-physics)
- [Radiopaedia - PET Imaging](https://radiopaedia.org/articles/positron-emission-tomography)
- [Questions and Answers in MRI](https://mriquestions.com/index.html)

---

<p align="center">
  <strong>SPARK Academy 2026</strong><br/>
  <em>Empowering the next generation of AI researchers in medical imaging across Africa</em>
</p>

<p align="center">
  <a href="https://github.com/SPARK-Academy-2025/SPARK-2026">GitHub</a> ·
  <a href="https://www.cameramriafrica.org/contact">Contact</a> ·
  <a href="https://www.cameramriafrica.org/spark">Website</a>
</p>